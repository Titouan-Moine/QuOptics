    ...
    for size in circuit_sizes:
        ...
        while not valid_results:
            ...
            if not sparse_allclose(result_A, result_B, atol=1e-10):
                ...
                # times_A.pop()  # Remove the last recorded time for this size since the results are invalid
                # times_B.pop()
                pass
            else:
                valid_results = True
            valid_results = True # for testing purposes, to avoid long benchmarking times while debugging

    # plt.figure(figsize=(9, 6))
    # plt.plot(circuit_sizes[1:], times_A[1:], color='blue', marker='o', linestyle='-', label='produit matriciel puis passage dans fock')
    # plt.plot(circuit_sizes[1:], times_B[1:], color='red', marker='s', linestyle='--', label='passage dans fock puis contraction de tenseurs')
