"""Module for generating random jump networks as unitary matrices or quimb TensorNetworks.
Contains functions to create beamsplitter and phase shifter gates, and to assemble them
into a random network with specified parameters. Also includes test cases for visualizing
...
def phaseshifter(m, phi, N):
    ...
    -------
    np.ndarray
        An N×N unitary phase shifter matrix with complex entries.
...
def add_gate(U, gate, islist=False):
    """
    ...
    - islist: whether U is a list of gates (default False)
...
def create_jump_random_network(n_modes, n_gate, islist=False, jump_size=1, theta_range=(0, math.pi/2), phi_range=(0, 2*math.pi), bs=True, ps=True, seed=None):
    """
    ...
    - islist: whether to return a list of gates instead of the full unitary (default False)
    ...
    for _ in range(n_gate):
        ...
        if ps & bs:
            ...
            if rng.random() < 0.5:
                ...
                add_gate(U, P, islist=islist)
            else:
                B = beamsplitter(i, j, phi, theta, n_modes)
                add_gate(U, B, islist=islist)
        elif bs:
            B = beamsplitter(i, j, phi, theta, n_modes)
            add_gate(U, B, islist=islist)
        elif ps:
...
def quimb_beamsplitter(m, n, mbis, nbis ,phi, theta, N, tags=None):
    '''
    ...
    if tags is None:
        ...
        tags = set(tags).union({'BEAMSPLITTER'})
...
def quimb_phaseshifter(m, mbis, phi, N, tags=None):
    if tags is None:
        tags = {'PHASESHIFTER'}
    else:
        tags = set(tags).union({'PHASESHIFTER'})
    T = np.array([
        [1, 0],
        [0, np.exp(1j * phi)],
    ], dtype=complex)
